...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    copyNForward: {
      async value(jid, message, forceForward = false, options = {}) {
        ...
        content[ctype].contextInfo = {
          ...(context || {}),
          ...(content[ctype]?.contextInfo || {}),
          mentionedJid: options.mentions || message.message[mtype]?.contextInfo?.mentionedJid || [],
          ...options
        }
        ...
        await conn.relayMessage(jid, waMessage.message, { messageId: waMessage.key.id, additionalAttributes: { ...options } })
        return waMessage
      },
      enumerable: true
    },
    ...
    insertAllGroup: {
      async value() {
        ...
        for (const group of Object.values(groups)) {
          conn.chats[group.id] = { ...(conn.chats[group.id] || {}), id: group.id, subject: group.subject, metadata: group }
        }
        return conn.chats
      }
    }
  ...
  // ─── PROTO UTILS ───────────────────────────────────────────────────────────
  if (sock.user?.id) sock.user.jid = sock.user.id

  return sock
}

// ─── PROTO TYPE EXTENSIONS ────────────────────────────────────────────────────
export function protoType() {
  ...
  Buffer.prototype.toBuffer = function toBuffer() {
    return this
  }
  ...
  String.prototype.isNumber = function isNumber() {
    const n = Math.abs(Number(this))
    return !isNaN(n) && String(n) === this
  }
...
export function serialize() {
  ...
  M.prototype.getQuotedObj = M.prototype.getQuotedMessage = async function getQuotedMessage() {
    if (!this.message) return false
    const q = ['senderKeyDistributionMessage', 'viewOnceMessage', 'documentWithCaptionMessage',
      'sendPaymentMessage', 'liveLocationMessage', 'requestPaymentMessage', 'declinePaymentRequestMessage',
      'cancelPaymentRequestMessage', 'viewOnceMessageV2', 'reactionMessage', 'pollCreationMessage',
      'pollUpdateMessage', 'keepInChatMessage', 'editedMessage', 'viewOnceMessageV2Extension']
    const mtype = Object.keys(this.message).find(v => !q.includes(v)) || Object.keys(this.message)[0]
    if (!mtype) return false
    const m = this.message[mtype]
    const quoted = this.quoted = m?.contextInfo?.quotedMessage || null
    if (!quoted) return false
    const qM = this.quoted = m.contextInfo.quotedMessage
    const type = Object.keys(qM)[0]
    const q2 = qM[type]
    this.quoted = smsg(store, { message: quoted, key: { remoteJid: this.key.remoteJid, fromMe: areJidsSameUser(m.contextInfo.participant, store.user?.id), id: m.contextInfo.stanzaId, participant: m.contextInfo.participant } })
    return this.quoted
  }
}
...
export function smsg(conn, m, hasParent) {
  ...
  if (m.message) {
    const noQ = ['senderKeyDistributionMessage','viewOnceMessage','documentWithCaptionMessage',
      'sendPaymentMessage','liveLocationMessage','requestPaymentMessage','declinePaymentRequestMessage',
      'cancelPaymentRequestMessage','viewOnceMessageV2','reactionMessage','pollCreationMessage',
      'pollUpdateMessage','keepInChatMessage','editedMessage','viewOnceMessageV2Extension']
    m.mtype = Object.keys(m.message).find(v => !noQ.includes(v)) || Object.keys(m.message)[0]
    ...
    const quoted = m.quoted = m.msg?.contextInfo?.quotedMessage
      ? smsg(conn, {
          key: {
            remoteJid: m.chat,
            fromMe: areJidsSameUser(m.msg.contextInfo.participant, conn.user?.id),
    ...
    if (m.quoted) {
      ...
      const vM = m.quoted.msg
      m.quoted.fakeObj = M.fromObject({
        key: { fromMe: m.quoted.fromMe, remoteJid: m.chat, id: m.quoted.id },
        message: m.quoted.message,
        ...(m.isGroup ? { participant: m.quoted.sender } : {})
      })
      m.quoted.download = () => conn.downloadMediaMessage(m.quoted.fakeObj)
      m.quoted.react = (e) => conn.sendMessage(m.chat, { react: { text: e, key: m.quoted.key } })
      m.quoted.reply = (text, jid, options) => conn.sendMessage(jid || m.chat, { text, ...options }, { quoted: m.quoted.fakeObj, ...options })
      m.quoted.copyNForward = (jid, forceForward, opts) => conn.copyNForward(jid, m.quoted.fakeObj, forceForward, opts)
    }
    ...
    m.name = !m.isGroup ? conn.getName(m.sender) : m.pushName
    m.reply = (text, jid, options) => conn.sendMessage(jid || m.chat, { text, ...options }, { quoted: m, ...options })
    m.react = (e) => conn.sendMessage(m.chat, { react: { text: e, key: m.key } })
    m.copyNForward = (jid, forceForward, opts) => conn.copyNForward(jid, m, forceForward, opts)
    m.download = m.msg?.url ? () => conn.downloadMediaMessage(m) : undefined
  }
  return m
}

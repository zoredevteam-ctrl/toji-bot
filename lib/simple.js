...
// ── 🛡️ FILTRO PARA EVADIR METADATOS DE WHATSAPP OFICIAL ──
const getContentType = (msg) => {
    if (!msg) return null;
    const keys = Object.keys(msg);
    // Ignoramos la basura de cifrado y contexto para ir al tipo real
    return keys.find(k => !['senderKeyDistributionMessage', 'messageContextInfo'].includes(k)) || keys[0];
};
...
export function smsg(conn, m) {
    ...
    if (m.message) {
        // 🔥 APLICAMOS EL FILTRO AL MENSAJE PRINCIPAL 🔥
        ...
        if (quoted?.quotedMessage) {
            ...
            // 🔥 APLICAMOS EL FILTRO AL MENSAJE CITADO 🔥
...
function extractBody(m) {
    ...
    switch (mtype) {
        ...
        case 'ephemeralMessage':
            return extractBody({ message: msg.ephemeralMessage?.message, mtype: getContentType(msg.ephemeralMessage?.message) })
        default:
            return ''
    }
}

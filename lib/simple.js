    ...
    sendCarousel: {
      async value(jid, text = '', footer = '', text2 = '', messages, quoted, options) {
        if (messages.length > 1) {
          ...
          }); // <--- REVISA QUE ESTO TENGA EL });

          msgs = await generateWAMessageFromContent(jid, messageContent, {
            userJid: conn.user.jid,
            quoted: quoted,
            upload: conn.waUploadToServer,
            ephemeralExpiration: 2000 // O la variable que tengas
          }); // <--- REVISA QUE ESTO TENGA EL });
...
}; // <--- AQUÍ ESTÁ EL TRUCO. El ";" y el ")" son vitales.
